...
| カラム | ソート | 備考 |
|---|---|---|
| 仕入先コード | ○ | `supplierCode`（支払先 = `MPaymentSupplier`） |
| 仕入先名 | ○ | `supplierName` |
| 税率 | - | `10%` / `8%` |
| 買掛金額（税込） | ○ | `taxIncludedAmountChange`、右寄せ |
...
`PageHeader` の actions に 2 つ:
- **再集計**: `POST /api/v1/finance/accounts-payable/recalculate?from=yyyy-MM-dd&to=yyyy-MM-dd`
  - 確認ダイアログ → `AccountsPayableSummaryJob` を同期 or 非同期起動
- **再検証**: `POST /api/v1/finance/accounts-payable/reverify?transactionMonth=yyyy-MM`
  - `AccountsPayableVerificationJob` 起動

※ バッチ起動は既存 `BatchController` 経由が既にあるため、そこにエンドポイントを足す形でも可。admin 権限のみ表示。
...
`t_accounts_payable_summary` に 2 カラム追加:
...
- `@Column(name = "verified_manually") private Boolean verifiedManually;`
...
export interface AccountsPayable {
  ...
  taxIncludedAmount: number | null  // 検証済み
...
|---|---|
| 締め日 | 20日固定（旧システム踏襲） |
| 手入力保護 | `verified_manually` フラグでバッチ上書きをブロック、admin のみ解除可 |
| 備考 | `verification_note` (VARCHAR 500) で DB 永続化 |
| バッチ起動方式 | 同期実行 + `@Transactional(timeout=60)` + 多重実行ガード（409） |
