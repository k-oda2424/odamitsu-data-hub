...
public class MPurchasePriceChangePlanService extends CustomService {
    ...
    /**
     * 自動生成され「作成」ステータスの見積に対応する仕入価格変更予定の作成済みフラグをリセットします。
     * バッチ再実行時に対象の見積を上書き再生成できるようにするためのガード解除処理です。
     *
     * @return リセット件数
     */
    @org.springframework.transaction.annotation.Transactional
    public int resetCreatedFlagForAutoGeneratedCreateEstimate() {
        return this.mPurchasePriceChangePlanRepository.resetCreatedFlagForAutoGeneratedCreateEstimate();
    }
