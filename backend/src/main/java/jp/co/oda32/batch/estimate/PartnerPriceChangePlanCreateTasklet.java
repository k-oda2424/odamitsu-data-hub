...
public class PartnerPriceChangePlanCreateTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        int resetPlanCount = mPurchasePriceChangePlanService.resetCreatedFlagForAutoGeneratedCreateEstimate();
        int resetPartnerPlanCount = mPartnerGoodsPriceChangePlanService.resetEstimateCreatedForAutoGeneratedCreateEstimate();
        if (resetPlanCount > 0 || resetPartnerPlanCount > 0) {
            log.info("再生成ガード解除: m_purchase_price_change_plan={}件、m_partner_goods_price_change_plan={}件をリセット",
                    resetPlanCount, resetPartnerPlanCount);
        }
        // ショップ毎に実行
    ...
    protected void createPartnerPriceChangePlan(MPurchasePriceChangePlan purchasePriceChangePlan) {
        ...
        for (MPartnerGoods mPartnerGoods : mPartnerGoodsList) {
            ...
            if (mPartnerGoods.getMCompany() == null) {
                log.warn("m_partner_goodsのcompany_no={}に該当するm_companyが存在しないためスキップ: shopNo={}, partnerNo={}, goodsCode={}",
                        mPartnerGoods.getCompanyNo(), mPartnerGoods.getShopNo(), mPartnerGoods.getPartnerNo(), mPartnerGoods.getGoodsCode());
                continue;
            }
            ...
            // 袋類など一桁単価(<10円)の商品は小数点2桁まで保持、通常商品は整数丸め
            int priceScale = mPartnerGoods.getGoodsPrice().compareTo(BigDecimal.TEN) < 0 ? 2 : 0;
            BigDecimal afterPrice = purchasePriceChangePlan.getAfterPrice().multiply(beforeProfitRate).setScale(priceScale, RoundingMode.HALF_UP);
